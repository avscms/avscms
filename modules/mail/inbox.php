...
if ( isset($_GET['delete']) ) {
    ...
    if ( $mail_id !== 0 ) {
        $sql    = "SELECT mail_id FROM mail
                   WHERE mail_id = " .$mail_id. " AND receiver = " .$conn->qStr($username). "
                   LIMIT 1";
        $conn->execute($sql);
...
$sql_count      = "SELECT COUNT(mail_id) AS total_messages FROM mail WHERE receiver = " .$conn->qStr($username). "
...
$sql            = "SELECT m.*, s.photo, s.gender
                   ...
                   WHERE m.receiver = " .$conn->qStr($username). "
