...
if ( isset($_POST['page']) ) {
    if ( isset($_SESSION['uid']) ) {
        ...
        $code[]         = '<span class="text-white">'. $lang['user.playlist'] .'</span> <strong>&middot;</strong>';
		...
		$code[] 		= '<button id="close_attach_mc_pv" type="button" class="close">&#215;</button>';
        $code[]         = '</div>';
        if ( $videos ) {
			$code[]	    = '<div class="m-b--15">';		
			$code[]     = '<div class="row">';				
            foreach ( $videos as $video ) {
                ...
                $code[] = '<a href="#attach_playlist_video_' .$video['VID']. '" id="attach_media_video_' .$video['VID']. '"><img src="' .get_thumb_url($video['VID']). '/1.jpg" alt="' .htmlspecialchars($video['title'], ENT_QUOTES, 'UTF-8'). '" class="img-responsive" /></a>';
                $code[] = '</div>';
            }
            $code[]     = '</div>';
            $code[]     = '</div>';		
