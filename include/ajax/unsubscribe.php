...
if ( isset($_POST['user_id']) ) {
    if ( isset($_SESSION['uid']) ) {
        ...
        if ( $uid == $user_id ) {
            $data['msg'] = show_err($lang['ajax.subscribe_self']);
        } else {
            $sql        = "SELECT UID FROM video_subscribe WHERE UID = " .$user_id. " AND SUID = " .$uid. " LIMIT 1";
            $conn->execute($sql);
            if ( $conn->Affected_Rows() != 1 ) {
                $data['msg'] = show_err($lang['ajax.unsubscribe_missing']);
            } else {
                ...
                $conn->execute($sql);
				$data['status'] = 1;
                $data['msg'] = show_msg($lang['ajax.unsubscribe_success']);
            }
        }
    } else {
        $data['msg'] = show_err($lang['ajax.unsubscribe_login']);
    }
} else {
