...
$data   = array('status' => 1, 'msg' => '');
    ...
    if ( isset($_SESSION['uid']) ) {
        ...
        $sql            = "SELECT p.PID, p.caption FROM photos AS p, photo_favorites AS f 
                           WHERE f.UID = " .$uid. " AND p.PID = f.PID AND p.type != 'private'";
        $rs             = $conn->execute($sql);
        $photos_t       = $rs->getrows();		
        ...
        $sql            = "SELECT p.PID, p.caption FROM photos AS p, photo_favorites AS f 
                           WHERE f.UID = " .$uid. " AND p.PID = f.PID  AND p.type != 'private' ORDER BY p.PID DESC LIMIT " .$limit;
        $rs             = $conn->execute($sql);
        $photos         = $rs->getrows();
        $page_link      = $pagination->getPagination('media_content', 'comments_favorite_photos_');
        ...
        $data['total'] = '<i class="fas fa-heart"></i> '.$lang['user.FAVORITE_PHOTOS'].': '.$total;	
        $data['status'] = 1;
        $data['code']   = implode("\n", $code);
    } else {
