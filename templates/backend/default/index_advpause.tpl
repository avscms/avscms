			...
			<div class="col-md-12">
				<div class="grid simple">
					<div class="grid-title no-border">
						<h4>In-Player Pause <span class="semi-bold">Ads</span></h4>
					</div>
					<div class="grid-body no-border">
						<div class="row">
							<div class="col-xs-12">
								<form class="form-no-horizontal-spacing search-filters" name="search_advertise" method="POST" action="index.php?m=advpause">
									...
									<div class="pull-right">
										...
										<button type="submit" name="search_pause" class="btn btn-success btn-cons btn-icon m-r-0"><i class="fa fa-search"></i></button>									
									</div>
										...
										<table class="table no-more-tables m-0">
											...
											<tbody>
												...
												<tr>
													...
													<td class="{if $smarty.section.i.index mod 2 == 0}grey{else}white{/if}">
														<a href="index.php?m=advpauseedit&AID={$advs[i].id}">{$advs[i].name|escape:'html'}</a>
													</td>
													<td class="{if $smarty.section.i.index mod 2 == 0}grey{else}white{/if}">
													...
													<td class="action {if $smarty.section.i.index mod 2 == 0}grey{else}white{/if}">
														<div>
															<a class="btn btn-success btn-xs btn-mini" href="index.php?m=advpauseedit&AID={$advs[i].id}">EDIT</a>
															{if $advs[i].status == '1'}
																<a class="btn btn-danger btn-xs btn-mini" href="index.php?m=advpause{if $page !=''}&page={$page}{/if}&a=suspend&AID={$advs[i].id}" onClick="javascript:return confirm('Are you sure you want to suspend this advertising pause?');">SUSPEND</a>
															{else}
																<a class="btn btn-success btn-xs btn-mini" href="index.php?m=advpause{if $page !=''}&page={$page}{/if}&a=activate&AID={$advs[i].id}" onClick="javascript:return confirm('Are you sure you want to activate this advertising pause?');">ACTIVATE</a>
															{/if}
															<a class="btn btn-danger btn-xs btn-mini" href="index.php?m=advpause{if $page !=''}&page={$page}{/if}&a=delete&AID={$advs[i].id}" onClick="javascript:return confirm('Are you sure you want to delete this advertising pause?');">DELETE</a>															
														</div>
													</td>									
												</tr>
												{/section}
