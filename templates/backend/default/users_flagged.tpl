	...
	<div class="page-content"> 
		<div class="content">
			...
			<div class="page-title">
				...
				<h3>Users - <span class="semi-bold">Flagged</span></h3>
			</div>
			...
			<div class="col-md-12">
				<div class="grid simple">
					...
					<div class="grid-body no-border">
						<div class="row">
							<div class="col-xs-12">
								<form class="form-no-horizontal-spacing search-filters" name="search_users" method="POST" action="users.php?m=flagged">
									<div class="filters">
										<div class="row">
											...
											<div class="filter col-xs-12 col-sm-6 col-md-4">
												<div class="form-group">
													<input type="text"  id="filter_title" name="name" value="{$option.name}" class="form-control {if $option.name}filter-active{/if}" placeholder="Full Name">
									...
									<div class="pull-left">
										<div class="btn-group"> <a class="btn dropdown-toggle btn-demo-space" data-toggle="dropdown" href="#">Order by <span id="sort_items">{if $option.sort == 'u.username'}Username{elseif $option.sort == 'u.email'}Email{elseif $option.sort == 'u.addtime'}Join Date{elseif $option.sort == 'u.logintime'}Last Login{elseif $option.sort == 'u.country'}Country{elseif $option.sort == 'u.profile_viewed'}Profile Views{else}ID{/if}</span> <span class="caret"></span> </a>
											<ul class="dropdown-menu">
												<li><a href="#" onClick="document.getElementById('sort_items').innerText = 'ID'; document.getElementById('sort').value = 'u.UID'" >ID</a></li>
												<li><a href="#" onClick="document.getElementById('sort_items').innerText = 'Username'; document.getElementById('sort').value = 'u.username'" >Username</a></li>
												<li><a href="#" onClick="document.getElementById('sort_items').innerText = 'Email'; document.getElementById('sort').value = 'u.email'" >Email</a></li>
												<li><a href="#" onClick="document.getElementById('sort_items').innerText = 'Join Date'; document.getElementById('sort').value = 'u.addtime'" >Join Date</a></li>
												<li><a href="#" onClick="document.getElementById('sort_items').innerText = 'Last Login'; document.getElementById('sort').value = 'u.logintime'" >Last Login</a></li>
												<li><a href="#" onClick="document.getElementById('sort_items').innerText = 'Country'; document.getElementById('sort').value = 'u.country'" >Country</a></li>																			
												<li><a href="#" onClick="document.getElementById('sort_items').innerText = 'Profile Views'; document.getElementById('sort').value = 'u.profile_viewed'" >Profile Views</a></li>									
											</ul>
										</div>									
									...
									<div class="pull-right">
										<button type="button" id="reset_search_flagged" name="reset_search_flagged" class="btn btn-white btn-cons btn-icon"><i class="fa fa-times"></i></button>									
								...
								<div>
									 {if $total_users >= 1}
										...
										<div>
											...
											<input type="submit" name="unflag_selected_users" value="Unflag" class="btn btn-white btn-cons" onClick="javascript:return confirm('Are you sure you want to unflag all selected users?');">											
										</div>
											...
											<div id="item-{$users[i].UID}" class="item-main-container small-thumb">
													...
													<div class="item-details">
														...
														<div class="row">						
															...
															<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3">
																...
																<a href="users.php?m=all&all=1&UID={$users[i].RID}" class="text-warning">{$uname}</a>														
															</div>	
															...
															<div class="col-xs-12 col-sm-4 col-md-3 col-lg-3">
																...
																<div class="text-red text-ellipsis">
																	<b>{$users[i].reason}{if $users[i].message}:{/if}</b>
																	{if $users[i].message}
																		<span alt="{$users[i].message|escape:'html'|nl2br}" title="{$users[i].message|escape:'html'|nl2br}">
																			{$users[i].message|escape:'html'|nl2br}
																		</span>
																	{/if}
																</div>																
															</div>
												...
												<div class="item-actions">																									
													<div class="btn-group">
														...
														{/if}
														<div class="btn-group">
															<a id="unflag__user_{$users[i].UID}" class="btn btn-success" data-toggle="dropdown" href="#" alt="Unflag" title="Unflag"><i class="fa fa-flag-o"></i></a>
															<ul class="dropdown-menu">
																<li><a id="unflag_user_{$users[i].flag_id}_{$users[i].UID}" href="#">Unflag</a></li>
															</ul>
														</div>														
													</div>
